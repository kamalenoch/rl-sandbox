...
export class SARSAAgent {
  ...
  update(state: string, action: Action, reward: number, nextState: string, nextAction: Action, done: boolean): void {
    const { alpha, gamma } = this.params;
    const q = this.getQ(state);
    const nextQ = this.getQ(nextState);
    const target = done ? reward : reward + gamma * nextQ[nextAction];
  ...
  runEpisode(env: GridEnvironment): { steps: number; totalReward: number; path: [number,number][]; success: boolean } {
    ...
    let state = env.stateKey(row, col);
    ...
    for (let step = 0; step < this.params.maxSteps; step++) {
      ...
      const nextState = env.stateKey(nextRow, nextCol);
      const nextAction = this.selectAction(nextState);
      this.update(state, action, reward, nextState, nextAction, done);
      row = nextRow; col = nextCol; state = nextState; action = nextAction; totalReward += reward;
      path.push([row, col]);
